...
import { rashiNames, nakshatraNames, vimshottariLords } from "../core/constants";
 ...
 */
export function getKpChart(date: Date, observer: Observer, config: KpConfig = {}): KpChart {
    ...
    const tropicalCusps = calculateTropicalPlacidusCusps(tropMC, tropAsc, observer.latitude, 23.4392911);
    ...
    // 4. Construct Houses
    const houses: KpHouse[] = cusps.map((cusp) => ({
        houseNumber: cusp.houseNumber,
        cusp,
        planets: [...cusp.planets],
    }));
    ...
    if (includeSignificators) {
        ...
        const primaryPlanets = ["Sun", "Moon", "Mars", "Mercury", "Jupiter", "Venus", "Saturn", "Rahu", "Ketu"];
        ...
        for (let h = 1; h <= 12; h++) {
            const occupants = cusps[h - 1].planets.filter((p) => primaryPlanets.includes(p));
        ...
        primaryPlanets.forEach((pName) => {
            ...
            for (let h = 1; h <= 12; h++) {
                ...
                if (houseSigMap[h].levelB.includes(starLord)) {
                    if (!houseSigMap[h].levelA.includes(pName)) houseSigMap[h].levelA.push(pName);
                ...
                if (houseSigMap[h].levelD.includes(starLord)) {
                    if (!houseSigMap[h].levelC.includes(pName)) houseSigMap[h].levelC.push(pName);
        ...
        primaryPlanets.forEach((pName) => {
            ...
            if (pSig) {
                const combined = Array.from(new Set([...pSig.levelA, ...pSig.levelB, ...pSig.levelC, ...pSig.levelD]));
...
export function formatKpChart(kpChart: KpChart): string {
    ...
    result += "Cusp | Degree       | Sign        | Sign Lord | Star Lord | Sub Lord  | Sub-Sub   | Planets in House\n";
    result += "-----------------------------------------------------------------------------------------------------\n";
    kpChart.cusps.forEach((c) => {
        const cNum = String(c.houseNumber).padStart(2, " ");
        const degStr = `${c.degree}°${String(c.minute).padStart(2, "0")}'${String(c.second).padStart(2, "0")}"`.padEnd(12, " ");
    ...
    result += "Planet  | Motion | Degree       | Sign        | Star Lord | Sub Lord  | Sub-Sub   | House | Into House\n";
    result += "----------------------------------------------------------------------------------------------------\n";
    Object.values(kpChart.planets).forEach((p) => {
        const pName = p.name.padEnd(7, " ");
        const motion = (p.isRetrograde ? "[R]" : "   ") + (p.isCombust ? "[C]" : "   ");
        const degStr = `${p.degree}°${String(p.minute).padStart(2, "0")}'${String(p.second).padStart(2, "0")}"`.padEnd(12, " ");
    ...
    if (kpChart.significators) {
        ...
        result += "House | Level A (Strongest) | Level B (Occupants) | Level C            | Level D (Lord)\n";
        result += "--------------------------------------------------------------------------------------------\n";
        for (let h = 1; h <= 12; h++) {
            const sig = kpChart.significators.houses[h];
            const hNum = String(h).padStart(2, " ");
            const aStr = (sig.levelA.length ? sig.levelA.join(", ") : "-").padEnd(19, " ");
            const bStr = (sig.levelB.length ? sig.levelB.join(", ") : "-").padEnd(19, " ");
            const cStr = (sig.levelC.length ? sig.levelC.join(", ") : "-").padEnd(18, " ");
            const dStr = sig.levelD.length ? sig.levelD.join(", ") : "-";
            result += `H${hNum}   | ${aStr} | ${bStr} | ${cStr} | ${dStr}\n`;
        }

        result += "\n--- KP PLANET SIGNIFICATORS (HOUSES SIGNIFIED) ---\n";
        result += "Planet  | Level A       | Level B       | Level C       | Level D       | All Houses\n";
        result += "------------------------------------------------------------------------------------\n";
        Object.values(kpChart.significators.planets).forEach((pSig) => {
            const pName = pSig.planetName.padEnd(7, " ");
            const aStr = (pSig.levelA.length ? pSig.levelA.join(",") : "-").padEnd(13, " ");
            const bStr = (pSig.levelB.length ? pSig.levelB.join(",") : "-").padEnd(13, " ");
            const cStr = (pSig.levelC.length ? pSig.levelC.join(",") : "-").padEnd(13, " ");
            const dStr = (pSig.levelD.length ? pSig.levelD.join(",") : "-").padEnd(13, " ");
 ...
 */
export function getKpPlanetInfo(kpChart: KpChart, planetName: string): KpPlanet | null {
 ...
 */
export function getPlanetsInKpHouse(kpChart: KpChart, houseNumber: number): string[] {
 ...
 */
export function getKpCuspInfo(kpChart: KpChart, houseNumber: number): KpCusp | null {
